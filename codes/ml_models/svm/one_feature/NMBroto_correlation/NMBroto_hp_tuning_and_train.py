...
label_90 = '../../../../../pca_features_label/round3/one_fearure/NMBroto_correlation/NMBroto_90_mixed_pca_label.tsv'
# ------------------- random hyper param grid ---------------
random_hyper_param_grid = svm_hpt.get_random_hyper_parameter_grid()
# ------------------------- get dfs -------------------------
NMBroto_90_X = get_data_frame_from_tsv_file(NMBroto_90)
NMBroto_90_X = NMBroto_90_X.iloc[:, 1:11]

label_90_Y = get_data_frame_from_tsv_file(label_90)
label_90_Y = label_90_Y.iloc[:, 1]
# -------------------------- train test split -------------------
x90_train, x90_test, y90_train, y90_test = utility.split_test_train(NMBroto_90_X, label_90_Y)
...
# ------------------------- save state -------------------------
utility.save_model("NMBroto_svm_after_hp1_best_model.sav", svm_random_model.best_estimator_)
pprint("model saved.")

model_result_log.svm_model_log("NMBroto_svm_after_hp1_model_log.txt",
...
svm_model_train = svm_classifier.train_predict_log(x_train=x90_train,
                                                   ...
                                                   log_filename='NMBroto_svm_rbf_eval_after_test_log.txt')
# --------------------------------------------------
utility.save_model("NMBroto_svm_rbf_model_train.sav", svm_model_train)
print("model saved.")
# --------------------------------------------------
